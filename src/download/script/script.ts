interface CustomFile {
  ...
  date: string;
  creator: string;
  platform: string; // Added platform property to identify DDC4000 vs DDC400
}

// Function to automatically scan the files directory and create file list
async function loadFilesFromDirectory() {
  try {
    const response = await fetch('/src/download/files-list.php');
    if (!response.ok) {
    ...
    return fileNames.map((fileName: string) => {
      ...
      return {
        ...
        date: "07.08.2024", // Default date
        creator: "RAT",     // Default creator
        platform
      };
    });
  } catch (error) {
...
function getHardcodedFiles(): CustomFile[] {
  return [
